...
unsigned int retstack_size) /* num entries in ret-addr stack */
{
	...
	case BPredPieceLin: // XXX: piecewise linear predictor
		pred->dirpred.piecewise = bpred_dir_create(class, l1size, l2size, shift_width, 0);
		break;
...
unsigned int xor) /* history xor address flag */
{
	...
	case BPredPieceLin: // XXX: piecewise linear predictor, resource allocation
		if (shift_width <= 0) { // TODO: additional configuration and/or runtime error checking code may be required
			fatal("shift register width, `%d', must be non-zero and positive", shift_width);
		} else {
			...
			// allocate w
			pred_dir->config.piecewise.w = (int ***) calloc(l1size, sizeof(int **));
			for (i = 0; i < l1size; ++i) {
...
FILE *stream) /* output stream */
{
	...
	case BPredPieceLin: // XXX: piecewise linear predictor
		fprintf(stream, "pred_dir: %s: \n\tpiecewise linear predictor with (n, m, h) = (%d, %d, %d)\n", name, pred_dir->config.piecewise.n, pred_dir->config.piecewise.m, pred_dir->config.piecewise.h);
		break;
...
FILE *stream) /* output stream */
{
	...
	case BPredPieceLin: // XXX: piecewise linear predictor
		bpred_dir_config(pred->dirpred.piecewise, "piecewise linear", stream);
		fprintf(stream, "btb: %d sets x %d associativity", pred->btb.sets, pred->btb.assoc);
		fprintf(stream, "ret_stack: %d entries", pred->retstack.size);
		break;
...
md_addr_t baddr) /* branch address */
{
	...
	case BPredPieceLin: { // XXX: piecewise linear predictor, prediction part
		int i;
		static unsigned char predict;

		// for easier access
		int ***w = pred_dir->config.piecewise.w;
		int *w00 = pred_dir->config.piecewise.w00;
		md_addr_t *ga = pred_dir->config.piecewise.ga;
		int *ghr = pred_dir->config.piecewise.ghr;
		int n = pred_dir->config.piecewise.n;
		int m = pred_dir->config.piecewise.m;
		int h = pred_dir->config.piecewise.h;
		int *output = &pred_dir->config.piecewise.output;

		// n was chosen as powers of 2 --> baddr % n
		md_addr_t baddr_mod = baddr & (n - 1);
		int ga_mod;
		*output = w00[baddr_mod];
		for (i = 0; i < h; ++i) {
			// m was chosen as powers of 2 --> ga[i] % m
			ga_mod = ga[i] & (m - 1);
			if (ghr[i])
				*output += w[baddr_mod][ga_mod][i];
			else
				*output -= w[baddr_mod][ga_mod][i];
		}
 ...
 * used on mispredict recovery */
{
	...
	case BPredPieceLin: // XXX: piecewise linear predictor, calls predict impl.
		if ((MD_OP_FLAGS(op) & (F_CTRL | F_UNCOND)) != (F_CTRL | F_UNCOND)) {
			dir_update_ptr->piecewise = bpred_dir_lookup(pred->dirpred.piecewise, baddr);
		}
		break;
	...
	if (pbtb == NULL) {
		...
		if (pred->class == BPredPieceLin) // XXX: piecewise linear predictor, returns prediction result
			return (*(dir_update_ptr->piecewise) ? /* taken */1 : /* not taken */0);
		else
			return ((*(dir_update_ptr->pdir1) >= 2) ? /* taken */1 : /* not taken */0);
	} else {
		/* BTB hit, so return target if it's a predicted-taken branch */
		if (pred->class == BPredPieceLin) // XXX: piecewise linear predictor, returns prediction result
			return (*(dir_update_ptr->piecewise) ? /* taken */pbtb->target : /* not taken */0);
		else
...
struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
	...
	/* train piecewise linear predictor */
	if (dir_update_ptr->piecewise) { // XXX: piecewise linear predictor, training part
		struct bpred_dir_t *pred_dir = pred->dirpred.piecewise;
		int i;
		...
		md_addr_t *ga = pred_dir->config.piecewise.ga;
		int *ghr = pred_dir->config.piecewise.ghr;
		float theta = pred_dir->config.piecewise.theta;
		int n = pred_dir->config.piecewise.n;
		int m = pred_dir->config.piecewise.m;
		int h = pred_dir->config.piecewise.h;
		int output = pred_dir->config.piecewise.output;
