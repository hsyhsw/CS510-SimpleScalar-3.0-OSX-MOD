...
enum bpred_class {
	...
	BPred_NUM, /* ?? */
	BPredPieceLin /* XXX: piecewise linear branch predictor */
};
...
struct bpred_dir_t {
	...
	union {
		...
		struct { // XXX: piecewise linear predictor, variables
			/**
			 * w: set of matrices, one for each branch,
			 *    whose column correspond to branches in the path history and
			 *    whose row correspond to positions in the history
			 *    see §3.2
			 */
			int ***w;
			/**
			 * w00: w[address, 0, 0] is the weight that keeps track of the tendency of a branch to be taken.
			 *      see §3.2
			 */
			int *w00;
			/**
			 * theta: parameter to decide when to stop updating the predictor.
			 *        see §3.2, §7.3
			 */
			float theta;
			int n;
			int m;
			int h; /* global history length */
			int *ghr; /* global history register */
			md_addr_t *ga; /* array of branch addresses */
			int output;
		} piecewise;
	} config;
};
...
struct bpred_t {
	...
	struct {
		...
		struct bpred_dir_t *piecewise; /* piecewise predictor */
	} dirpred;
...
struct bpred_update_t {
	...
	char *piecewise; /* XXX: piecewise predictor counter */
	struct { /* predicted directions */
		...
		unsigned int piecewise :1; /* XXX: piecewise linear predictor */
	} dir;
};
